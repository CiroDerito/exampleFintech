...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSuccess('');
    try {
      await api.post('/users', { email, name, password, organizationId });
    ...
    <main className="min-h-screen flex w-full bg-gray-200">
      ...
      <div className="w-full md:w-1/2 flex justify-center items-center h-screen">
        <form className="p-8 bg-white rounded shadow w-80" onSubmit={handleSubmit}>
          ...
          <input type="text" placeholder="ID Organización" className="w-full mb-2 p-2 border rounded" value={organizationId} onChange={e => setOrganizationId(e.target.value)} required />
